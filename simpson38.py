...
raiz = Tk()
raiz.title("Calculadora WmU Simpson 3/8")
...
#Titulo
titulo = Label(raiz,text=" Calculadora - Método de Simpson 3/8", font=("Impact",20), fg="rosybrown", bg="black")
titulo.pack(pady=20)
...
def calcular():
    try:
        ...
        valorMetodo = (3*h/8)*(f(x0)+3*f(x1)+3*f(x2)+f(x3))
        error= (-3*(h**5)/80)*(f4(e)) 
        ...
        resp = Label(ventana_resultado,text="Resultado", font=("Times New Roman",16,"bold"), bg="black", fg="rosybrown")
        resp.pack(pady=20)
        ...
        boton_cerrar = Button(ventana_resultado, text="Cerrar", command=ventana_resultado.destroy, fg="black", bg="rosybrown")
...
boton = Button(raiz, text="Calcular", command=calcular, bg="rosybrown")
boton.pack(pady=25)
